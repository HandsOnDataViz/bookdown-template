...
Header list (always insert blank line unless directly after hashtag header)
...
- third-level header (C) = 3 hashtags followed by `{-}` to avoid numbering
- fourth-level header (D) = 4 hashtags followed by `{-}` to avoid numbering
...
### Cross-references {-}
The cross-reference strategies below require unique names to be assigned to each chapter, section, figure, and table.

- All cross-refs are clickable and auto-numbered (except chapter) in HTML.
- All (except chapter) are clickable and auto-numbered in PDF.
- All are auto-numbered (except chapter) in Word, but none are clickable.
- TBA with AsciiDoc.

Demos:

- To another *chapter* with HTML link: See [chapter 2](chapter2.html).
- To another *section* with Bookdown ID ref: See section \@ref(style-guide).
- To figure in any chapter/section with Bookdown ID ref: Figure \@ref(fig:sample-interactive-map), or Figure \@ref(fig:sample-static-image2).
- To table in any chapter/section with Bookdown ID ref: Table \@ref(tab:unique1).

Bookdown: If you want to cross-reference figures or tables generated from a code chunk, please make sure the chunk label only contains _alphanumeric_ characters (a-z, A-Z, 0-9), slashes (/), or dashes (-).
...
Demo: Simple HTML code comment in .Rmd file (does *not* appear in MS Word or PDF or AsciiDoc):
